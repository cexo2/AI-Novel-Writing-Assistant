...
卷重置后可以显式重建本卷章节同步：当某一卷推倒重做并重新拆出章节后，可以只重建当前卷的章节执行映射，旧章节会先保留在快照历史中，再按当前拆章结果替换为新的执行章节。

- 普通“保存卷工作区”和“修复章节连接”继续保持保守策略，不会默认删除已有正文或改动其它卷。
- “重建本卷章节同步”只作用于当前选中的卷，会刷新本卷拆章与章节执行区的 chapterId 映射。
- 如果当前卷章节正在生成、流水线或自动导演任务仍在占用，系统会阻止重建，避免出现半替换状态。
